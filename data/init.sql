CREATE DATABASE hindi;
ALTER DATABASE hindi CHARACTER SET utf8 COLLATE utf8_unicode_ci;
use hindi;

CREATE TABLE hindi_lexicon (
    id INT NOT NULL AUTO_INCREMENT,
    word_dev NVARCHAR(30), /* word in Devanagari */
    pos NVARCHAR(30), /* part of speech */
    word_eng VARCHAR(30), /* word translation in English */
    definition NVARCHAR(255), /* word definition in Devanagari */
    PRIMARY KEY (id)
);

INSERT INTO hindi_lexicon
    (word_dev, pos, word_eng, definition)

VALUES
    (N'स्कूल', NULL, 'school', NULL),
    (N'पहला', NULL, 'first', NULL),
    (N'दन', NULL, 'day', NULL),
    (N'मेरे', NULL, 'my', NULL),
    (N'दोत', NULL, 'friend or friends', NULL),
    (N'आम', NULL, 'mango', NULL),
    (N'का', NULL, 'possessive identifier', NULL),
    (N'पेड़', NULL, 'tree', NULL)
;

CREATE TABLE hindi_story (
    id NVARCHAR(1) NOT NULL,
    title NVARCHAR(30), /* word in Devanagari */
    story LONGTEXT, /* part of speech */
#     word_eng VARCHAR(30), /* word translation in English */
#     definition NVARCHAR(255), /* word definition in Devanagari */
    PRIMARY KEY (id)
);

INSERT INTO hindi_story
    (id,title, story)

VALUES
    ('A', 'First Day of School', 'स्कूल पहला दन\n\
Author: Cheryl Rao\n\
Illustrator: Mayur Mistry\n\
Translator: Arti Smit\n\
\n\
स्कूल में आज मेरा पहला दन है। माँ मेरा हाथ पकड़े हए हैं और मेरे साथ चल रही ह ।\n\
“मैं अब बड़ी हो गई हूँ,” मैं कहती हूँ।\n\
“चलो...चलो!”\n\
माँ ने मेरा हाथ कसकर पकड़ा हआ है।\n\
\n\
स्कूल के पास बत सारे बचे ह।\n\
कुछ बस से आते हैं।\n\
कुछ रिक्शी से आते हैं।\n\
कुछ साइकिल से और।\n\
कुछ पैडल आते हैं, मेरी तरह।\n\
\n\
हम फाटक तक पहुँचे।\
माँ मेरा हाथ छोड़ती हैं।\
वह गेट पर रुक गईं। मुझे अंदर अके अकेले जाना है।\
मेरे चारीं तरफ़ बहुत से अनजाने चेहरे हैं।\
\n\
मैं एक क़दम चलती हूँ। मैं दूसरा क़दम बढ़ाती हूँ। मैं पीछे मुड़कर देखती हूँ।\
जैसे मैं आगे बढ़ती जाती हूँ, माँ छोटी दिखाती जाती हूँ।\
क्या वह गायब हो जाएँगी? मैं दौड़कर उनके पास जाती हूँ।\
मुझे नहीं लगता कि मैं बड़ी हो चुकी हूँ। मैं उनका हाथ पकड़ती हूँ और कहती हूँ, “मत जाओ!”\
\n\
सभी अंदर जा चुके हैं। सिर्फ मैं बाहर हूँ। टीचर दीदी बाहर आती हैं।\
वे मुझे देख मुस्कराती हैं। मैं भी मुस्कराती हूँ।\
माँ कहती हैं, “रानी जब तुम बाहर आओगी मैं यहीं मिलूँगी।”\
मैं उनका हाथ छोड़ देती हूँ। वह हाथ हिलाती हैं।\
\n\
मैं दौड़कर अंदर जाती हूँ।\n\
माँ छुट्टी होने पर वहीं मिलेंगी!\n\
\n\
Attribution Text: स्कूल का पहला दिन (Hindi),\n\
translated by Arti Smit, based on original story\n\
Rani\'s First Day at School (English), written by Cheryl Rao,\n\
illustrated by Mayur Mistry, published by Pratham Books\n\
(© Pratham Books, 2015) under a CC BY 4.0 license on StoryWeaver.\n\
Read, create and translate stories for free on www.storyweaver.org.in'),
    ('B', 'My Friends', 'मेरे दोस्त\n\
Author: Rukmini Banerji\n\
Illustrator: Rajeev Verma ''Banjara''\n\
\n\
मेरे बहुत सारे दोस्त हैं।\n\
मेरे कु छ दोस्त बड़े-बड़े हैं। और...\n\
कुछ दोस्त छोटे हैं।\n\
मेरे कई दोस्त बूढ़े भी हैं।\n\
और... कुछ नन्हे-मुन्ने भी हैं।\n\
मेरे कु छ दोस्त ऐसे भी हैं जिनकी...पूँछ भी है।\n\
और कुछ ऐसे जिनके... पैर ही नहीं हैं।\n\
मेरे कुछ दोस्त उड़ते हैं। और कुछ दोस्त तैरते भी हैं।\n\
ओह! हो! किताबों भी तो मेरी दोस्त हैं।\n\
लेकिन, मेरा सबसे अच्छा दोस्त कौन है?\n\
कौन? कौन? कौन?\n\
\n\
मेरी माँ!\n\
\n\
Attribution Text: मेरे दोस्त (Hindi), written by Rukmini Banerji,\n\
 illustrated by Rajeev Verma ''Banjara'', published by Pratham Books (© Pratham Books, 2006) \n\
 under a CC BY 4.0 license on StoryWeaver. Read, create and translate stories for free on www.storyweaver.org.in'),
    ('C', 'The Mando Tree', 'आम का पेड़\n\
Author: Kanchan Bannerjee\n\
Illustrator: Kavya Singh\n\
Translator: Rajesh Khar\n\
 \n\
हमें नानी के गाँव जाना अच्छा लगता है।\n\
नानी के बगीचे में एक बड़ा सा आम का पेड़ है।\n\
उस पर बहुत सी चिड़ियाँ आती है।\n\
\n\
मुझे आम के पेड़ पर छुपना बड़ा अच्छा लगता है।\n\
यहाँ कोई मुझे ढूँढ नहीं पाता।\n\
\n\
बाबा मुझे कुएँ के पास खोजते हैं।\n\
माँ गौशाला के आस-पास ढूँढती हैं।\n\
पर मैं किसी को नहीं मिलती।\n\
\n\
अरे, नहीं!\n\
नानी का कुत्ता मुझे ढूँढ निकालता है।\n\
\n\
तब मुझे नीचे जाना ही पड़ता है।\n\
नानी मुझे ज़ोर से गले लगा लेती हैं।\n\
\n\
Attribution Text: आम का पेड़ (Hindi), translated by Rajesh Khar, \n\
based on original story The Mango Tree (English), written by Kanchan Bannerjee,\n\
illustrated by Kavya Singh, published by Pratham Books (© Pratham Books, 2013) \n\
under a CC BY 4.0 license on StoryWeaver.\n\
Read, create and translate stories for free on www.storyweaver.org.in')
;